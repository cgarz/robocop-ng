...
class NkitMeme(Cog):
    ...
    @commands.check(check_if_staff_or_ot)
    @commands.command(hidden=True, aliases=["thank", "nkitgold"])
    ...
    @commands.command(hidden=True)
    async def whovian(self, ctx):
        """test"""
        await ctx.send("Whovian, stop spamming the testing channel, goddamn it. Signed, Whovian.")


async def setup(bot):
    await bot.add_cog(NkitMeme(bot))
